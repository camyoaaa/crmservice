...
const { generateConditions } = require("../tool");
...
router.get("/list", async function (req, res, next) {
    ...
    try {
        ...
        const result = await neworderModel.aggregate([
            {
                //关联售后记录
                $lookup: {
                    from: "Aftersales",
        ...
        let count = result[0].count[0] ? result[0].count[0].count : 0;
        res.json({
            status: 200,
            message: "获取成功",
            timestamp: Date.now(),
